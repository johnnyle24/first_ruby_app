...
class GameHelperTest < Minitest::Test
  ...
  def test_contains
    ...
    selected_terms.each do |term|
      assert term.match(/[#{letters.join}]/),
             "#{term} does not contain #{letters.join(',')}"
    end
  end
  ...
  def test_does_not_contain
    ...
    selected_terms.each do |term|
      assert term.match(/\A[^#{letters.join}]*\z/),
             "#{term} contains #{letters.join(',')}"
    end
  end

  def test_case_1
    terms = @gh.all_words.with_word_length(5)
               .begins_with('e').does_not_contain('x')
    assert terms.all? { |term| term.size == 5 && term.match(/\Ae[^x]{4}\z/) }
  end

  def test_case_2
    terms = @gh.all_words.with_word_length(6)
               .begins_with('e', 'a').does_not_contain('y', 'i')
    assert terms.all? { |term|
      term.size == 6 &&
        term.match(/\A[ea][^iy]{5}\z/)
    }
  end

  def test_case_3
    terms = @gh.all_words.with_word_length(6)
               .begins_with('e').char_count_less_than('e', 2)
               .does_not_contain('y')
    assert terms.all? { |term|
      term.size == 6 &&
        term.match(/\Ae[^ey]{5}\z/)
    }
  end

  def test_case_4
    terms = @gh.all_words.with_word_length(4, 5)
               .contains('y').does_not_end_with('y')
    assert terms.all? { |term|
      [4, 5].include?(term.size) && term.match(/y/) &&
        term.match(/[^y]\z/)
    }
  end
end
